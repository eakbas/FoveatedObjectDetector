...
for i=1:length(imgids)  
    ...
    mu_this = zeros(length(mu), length(model.scales));
    n_this = zeros(length(model.scales),1);
    parfor s=1:length(model.scales)
        ...
        F = return_all_feature_vectors(feats, th, tw);
        F2 = return_all_feature_vectors(flipfeat(feats), th, tw);
...
%         end
        ...
        mu_this(:,s) = sum(F,2) + sum(F2,2);
        
        n_this(s) = size(F,2) + size(F2,2);
    end
...
%         end
        ...
        textprogressbar(100*((i-1)*length(model.scales)+s)/ ...
            (length(model.scales)*length(imgids)));
    end
end
cov = cov./n;
textprogressbar(-1);
