...
%         model.templates(t).B = 0;
    ...
    % use whitened features 
    L = sqrtm(C0s{t});
    % following two lines is WRONG!! inv(R) must be used
    whitened_orig = (L\bsxfun(@minus, warped_pos_feats{t,1}, mu0s{t}')')';
    whitened_flipped = (L\bsxfun(@minus, warped_pos_feats{t,2}, mu0s{t}')')';
    
        % find best initialization (i.e. minimum total variation in clustering)
        tv = zeros(min(1000,size(warped_pos_feats{t,1},1)),1);
        for f=1:length(tv)            
            [~,tv(f)] = left_right_clustering(whitened_orig, ...
                whitened_flipped, f);
        end
        [~,id] = min(tv);
        [~,~,to_left] = left_right_clustering(whitened_orig, ...
            whitened_flipped,id);
        feats = [warped_pos_feats{t,1}(to_left,:); ...
            warped_pos_feats{t,2}(~to_left,:)];
        % }
...
%         model.templates(t).B = -mean(feats,1)*model.templates(t).w;
